...
export default function airingGo(chessBoard: []) {
  if (over) {
    return;
  }
  ...
  console.log("airingGo -> max", max);

  //   oneStep(u, v, false);
  console.log("airingGo -> u, v", u, v);
  //   chessBoard[u][v] = 2;

  for (var k = 0; k < count; k++) {
    console.log("airingGo -> wins[u][v][k]", wins[u][v][k]);
    if (wins[u][v][k]) {
      myWin[k]++;
      console.log("airingGo -> myWin", myWin);
      airingWin[k] = 6;
      if (myWin[k] == 5) {
        // window.alert("Win!");
        over = true;
